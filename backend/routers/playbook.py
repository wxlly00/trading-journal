...
@router.get("/learning")
async def list_learning(category: str | None = None, user: dict = Depends(get_current_user)):
    db = get_client()
...
async def create_learning(payload: dict, user: dict = Depends(get_current_user)):
    db = get_client()
    return db.table("learning_entries").insert({**payload, "user_id": user["sub"]}).execute().data[0]


@router.patch("/learning/{entry_id}")
async def update_learning(entry_id: str, payload: dict, user: dict = Depends(get_current_user)):
    db = get_client()
    db.table("learning_entries").update(payload).eq("id", entry_id).eq("user_id", user["sub"]).execute()
    return {"ok": True}
...
@router.get("/drawings")
async def list_drawings(user: dict = Depends(get_current_user)):
    db = get_client()
    return db.table("drawings").select("id,title,tags,created_at,updated_at").eq("user_id", user["sub"]).order("updated_at", desc=True).execute().data


@router.post("/drawings")
async def create_drawing(payload: dict, user: dict = Depends(get_current_user)):
    db = get_client()
    return db.table("drawings").insert({**payload, "user_id": user["sub"]}).execute().data[0]
...
async def update_drawing(drawing_id: str, payload: dict, user: dict = Depends(get_current_user)):
    db = get_client()
    db.table("drawings").update(payload).eq("id", drawing_id).eq("user_id", user["sub"]).execute()
    return {"ok": True}
